...
*** Early goal declaration
		...
		#+ATTR_REVEAL: :frag (appear)
		 - When dereferencing a node many answers can become available
			 - Search algorithms declare goals when removing a node from the queue
			 - Removing nodes from the queue may include a dereference
			 - But the data was available when inserting those nodes in the queue
		 - Search can be tweaked to announce answers as soon as they can be proved
			 - And the heuristic can be fixed to consider this reward change
